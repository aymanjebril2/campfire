    ...
    const response = await axios
      ...
      .then((res) => {
        ...
        if (secss) {
          ...
          localStorage.setItem("user-info", JSON.stringify(secss));
          history.push("/login");
        }
      })
      .catch((error) => {
        ...
        if (err) {
          setErrMsg(err.firstName || err.lastName || err.email || err.password);
        }
      });
  };
          ...
          <form className={classes.form} onSubmit={handleSubmit}>
            <Grid container spacing={2}>
              <ThemeProvider theme={theme}>
                ...
                <Grid item xs={12}>
                  <Tooltip
                    title="Passphrase must be 16 characters long"
                    placement="left"
                  >
                    <TextField
                      color="primary"
